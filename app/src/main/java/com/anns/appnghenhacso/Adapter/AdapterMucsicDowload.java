...
import com.anns.appnghenhacso.Fragment.FragmentFavorite;
...
public class AdapterMucsicDowload extends RecyclerView.Adapter<AdapterMucsicDowload.ViewHolder> {
    ArrayList<Item> items;
    Context context;
    boolean status_favorite;

    public AdapterMucsicDowload(ArrayList<Item> items, Context context) {
        this.items = items;
    ...
    @NonNull
    @Override
    public AdapterMucsicDowload.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        LayoutInflater inflater = LayoutInflater.from(parent.getContext());
        View view = inflater.inflate(R.layout.item_list_category,parent,false);
        return new AdapterMucsicDowload.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull AdapterMucsicDowload.ViewHolder holder, int position) {
        ...
        Bitmap scaledBitmap = BitmapFactory.decodeFile(items.get(position).getFields().getImage_Url(), options);
        holder.list_category_image.setImageBitmap(scaledBitmap);
                        ...
                        .setPositiveButton("Chắc chắn", new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                if(value!=null){
                                    ...
                                    File file_img = new File (items.get(position).getFields().getImage_Url());
                                    if(file_img.exists()){
                                        file_img.delete();
                                    }
                                    File file_mp3 = new File(items.get(position).getFields().getMusic_Url());
                                    if(file_mp3.exists()){
                                        file_mp3.delete();
                                    }
